...
const HelpSupport = () => {
  const [form, setForm] = useState({ name: '', email: '', concernType: '', message: '' });
          ...
          <form onSubmit={handleSubmit} className="p-8">
            <div className="space-y-5">
              <div>
                ...
                <input type="text" name="name" value={form.name} onChange={handleChange} required disabled={loading} className={inputClass} placeholder="John Doe" />
              </div>

              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Your Email</label>
                <input type="email" name="email" value={form.email} onChange={handleChange} required disabled={loading} className={inputClass} placeholder="john@example.com" />
              </div>

              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Concern Type</label>
                <div className="relative">
                  <select name="concernType" value={form.concernType} onChange={handleChange} required disabled={loading} className={inputClass + " appearance-none cursor-pointer"}>
              ...
              <div>
                ...
                <textarea name="message" value={form.message} onChange={handleChange} required disabled={loading} rows="5" className={inputClass + " resize-none"} placeholder="Please provide details about your concern..." />
              </div>
            </div>

            <button type="submit" disabled={loading} className="w-full mt-7 bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white font-bold py-4 px-6 rounded-xl shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none flex items-center justify-center focus:outline-none focus:ring-4 focus:ring-green-300">
              {loading ? (<><svg className="animate-spin h-5 w-5 mr-3" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>Submitting...</>) : (<><svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8" /></svg>Submit Request</>)}
            </button>

            <div className="mt-6 pt-6 border-t border-gray-200 text-center">
              <p className="text-sm text-gray-600 mb-2">Need immediate assistance?</p>
              <div className="flex flex-wrap justify-center gap-x-4 gap-y-2 text-sm">
                <a href="tel:+1234567890" className="text-green-600 hover:text-green-700 font-semibold hover:underline transition-colors">📞 +123-456-7890</a>
                <span className="text-gray-400">|</span>
                <a href="mailto:support@turf.com" className="text-green-600 hover:text-green-700 font-semibold hover:underline transition-colors">✉️ support@turf.com</a>
              </div>
            </div>
          </form>
        </div>
